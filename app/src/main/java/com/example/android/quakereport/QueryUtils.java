...
public final class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
            ...
            JSONArray jsonArrayFeatures = baseJsonRespone.optJSONArray("features");

            for(int i=0; i<jsonArrayFeatures.length();i++){
                JSONObject jsonObjectFeature = (JSONObject) jsonArrayFeatures.get(i);
                JSONObject jsonObjectProperties = jsonObjectFeature.optJSONObject("properties");
                Double mag = jsonObjectProperties.optDouble("mag");
                String location = jsonObjectProperties.optString("place");
